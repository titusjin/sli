...
export function callFillMoreUserInfo (data, proyes) {
  ...
  return (dispatch, getState) => {
    ...
    queryObj.accessToken = getState().RegistDiverFormReducerExt.accessToken
    queryObj.data = {
...
export function callSaveProInfo (data) {
  ...
  return (dispatch, getState) => {
    ...
    _.forEach(getState().RegistDiverFormReducerExt.certMetas, function (o) {
    ...
    if (data.verifyCode) {
      queryObj.data.verifyCode = data.verifyCode
    }
    ...
    upLoadToS3(dispatch, getState, data.licenseFontPic)
            .then(res => {
              ...
              if (queryObj.data.verifyCode) {
                return userAPI.handleProExt(queryObj)
              } else {
